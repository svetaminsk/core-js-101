...
function doRectanglesOverlap(rect1, rect2) {
  if (rect2.top > rect1.width || rect2.left > rect1.height
    || rect1.top > rect2.width || rect1.left > rect2.height) {
    return false;
  } return true;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  while (bool) {
    const tmp = path;
    const cond = pathes.every((e) => e.startsWith(tmp));
    if (cond && (path.at(path.length - 1) === '/')) return path;
    path = path.slice(0, -1);
    if (path.length === 0) {
      bool = false;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let column = 0; column < 3; column += 1) {
    if ((position[0][column] === position[1][column])
    && (position[1][column] === position[2][column])
    && (position[1][column] !== undefined)) {
      return position[2][column];
    }
  }
